class GraphEdge {
    ...
    drawCurved() {
        ...
        let startp = {
            x: this.sourceNode.X - ((fac * distance) * Math.sin(angle - 4 * Math.PI / 4)),
            y: this.sourceNode.Y + ((fac * distance) * Math.cos(angle - 4 * Math.PI / 4))
        }
        let endp = {
            x: this.targetNode.X - ((fac * distance) * Math.sin(angle + 4 * Math.PI / 4)),
            y: this.targetNode.Y + ((fac * distance) * Math.cos(angle + 4 * Math.PI / 4))
        }
        //Curve mid points 

        let p2 = {
            x: this.sourceNode.X + ((distance / 3) * Math.sin(angle - (2 * Math.PI / 4 + rot))),
            y: this.sourceNode.Y - ((distance / 3) * Math.cos(angle - (2 * Math.PI / 4 + rot)))
        }
        let p3 = {
            x: this.targetNode.X + ((distance / 3) * Math.sin(angle + (2 * Math.PI / 4 + rot))),
            y: this.targetNode.Y - ((distance / 3) * Math.cos(angle + (2 * Math.PI / 4 + rot)))
        }
        ...
        */
        ...
        let vect = GraphEdge.middleOfLine(p2.x, p2.y, p3.x, p3.y);
        ...
        text(this.cost, vect.X, vect.Y - 6);
    }
